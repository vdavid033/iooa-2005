<template>
  <q-page class="q-pa-md" style="position: relative; overflow: hidden">
    ...
    <!-- Glavni container, širok 80%, sa marginom samo lijevo -->
            ...
            <table cellpadding="6" cellspacing="0" style="border-collapse: collapse; width: 100%;">
              <thead>
                <tr>
                  ...
                  <th class="akcija-col" style="width: 160px;"></th> <!-- Šira kolona za akcijske gumbe -->
              ...
              <tbody>
                <tr v-for="u in lockedUsers" :key="u.id_korisnika">
                  ...
                  <td class="akcija-col" style="background-color: #ffe6e6;">
                    <q-btn
                    label="Otključaj"
                    color="primary"
                    dense
          ...
          <table cellpadding="6" cellspacing="0" style="border-collapse: collapse; width: 100%;">
            ...
            <tbody>
              <tr v-for="u in unlockedUsers" :key="u.id_korisnika">
                ...
                <td class="bordered" style="">{{ u.adresa || '-' }}</td>
...
async function loadUsers() {
  ...
  try {
    ...
    if (data.error === false) {
      // Izbaci trenutno prijavljenog korisnika iz prikaza
...
async function toggleLock(u) {
  ...
  try {
    ...
    $q.notify({ type: 'negative', message: 'Greška na serveru.' })
  } finally {
    removeProcessing(id)
  }
}
...
async function resetPassword(u) {
  ...
  try {
    ...
    if (data.error === false) {
      $q.notify({ type: 'positive', message: `Lozinka korisnika ${u.korisnicko_ime} resetirana na 123456.` })
    } else {
      $q.notify({ type: 'negative', message: data.message || 'Greška pri resetiranju lozinke.' })
    }
  } catch (err) {
...
/* Apply borders to all table cells except Akcija */
.bordered {
  border: 1px solid #ccc;
}

/* Remove borders from Akcija column */
td.akcija-col {
